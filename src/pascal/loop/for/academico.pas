...
begin
	...
	for cont:= 1 to MAX do
	begin
		if (aluno[cont].media >= 60) and (aluno[cont].frequencia >= 80) then
		begin 
			writeln(aluno[cont].nome);
		end;
	end;
...
begin
	...
	for cont:= 1 to MAX do
	begin
		if (aluno[cont].media < 40) or (aluno[cont].frequencia < 80) then 
		begin
			writeln(aluno[cont].nome);
		end;
	end;
...
begin
	...
	for cont:= 1 to MAX do
	begin
		if (aluno[cont].media <= 40) and (aluno[cont].media > 60) or (aluno[cont].frequencia < 80) then 
		begin
			writeln(aluno[cont].nome);
		end;
	end;
